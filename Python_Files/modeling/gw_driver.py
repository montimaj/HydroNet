...
from Python_Files.datalibs.sysops import makedirs, make_proper_dir_name, copy_files
from Python_Files.modeling import ml_driver as mld
...
class HydroML:
    ...
    def create_dataframe(self, year_list, column_names=None, ordering=False, load_df=False, exclude_vars=(),
        ...
        self.ml_data_dir = make_proper_dir_name(self.file_dir + 'ML_Data')
        ...
        if load_df:
            ...
            print('Copying files...')
            makedirs([self.ml_data_dir, self.pred_data_dir])
            input_dir_list = [self.final_gw_dir] + [self.raster_mask_dir]
            pattern_list = [pattern] * len(input_dir_list)
            copy_files(input_dir_list, target_dir=self.ml_data_dir, year_list=year_list, pattern_list=pattern_list,
                       verbose=verbose)
            copy_files([self.crop_coeff_mask_dir], target_dir=self.ml_data_dir, year_list=year_list,
                       pattern_list=[pattern], rep=True, verbose=verbose)
            copy_files([self.lu_mask_dir], target_dir=self.ml_data_dir, year_list=year_list,
                       pattern_list=[pattern], rep=True, verbose=verbose)
            ...
            df = mld.create_dataframe(self.ml_data_dir, out_df=df_file, column_names=column_names, make_year_col=True,
    ...
    def get_predictions(self, fitted_model, pred_years, column_names=None, ordering=False, pred_attr='GW',
        ...
        actual_raster_dir = self.ml_data_dir
        if use_full_extent:
...
def run_gw_ks(load_files=True, load_df=False, verbose=True):
    """
...
def run_gw_az(load_files=True, load_df=False, verbose=True):
    """
...
def create_ml_data(load_files=True, load_df=True, verbose=True):
    """
    Create ML data using both Kansas and Arizona data
    :param load_files: Set True to load existing files, needed only if analyze_only=False
    :param load_df: Set True to load existing dataframe from CSV
    :param verbose: Set False to hide extra info
